...
```
Llama la atención que en algunos valores faltantes, como la primera observación ordenadas de menor a mayor en Sepal.Length es 0 en lugar de NA, lo cual es imposible ya que por pequeña que sea algun valor debería de tener.
Por otro lado, cuando ordenas de menor a mayor Sepal.Width la primera observación es negativa y la segunda 0, siendo ambos casos imposibles, ya que deberían ser o NA o un valor positivo.
Sepal.Length ordenado de menor a mayor, de nuevo presenta un valor de 0 en la primera observación.

Cuando ordenamos de mayor a menor, obtenemos valores muy grandes que desentonan demasiado, haciendo pensar que se tratan de outliers. El mejor ejemplo es cuando ordenamos Petal.Width de mayor a menor y vemos que en la primera columna tiene un valor de infinito, siendo también imposible.
...
num_completas <- sum(complete.cases(AB_iris))
porc_completas <- mean(complete.cases(AB_iris)) * 100

lapply(AB_iris,complete.cases)

clean_iris <- na.omit(AB_iris) 
...
MADM <- function(x) {
  1.4826 * median(abs(x - median(x, na.rm = TRUE)), na.rm = TRUE)
}
...
```
La media para Petal.Length es mayor es en el conjunto dirty.
La desviacion típica para Petal.Length es considerablemente mayor en el conjnto dirty, debido a la existencia de valores extremos.
La mediana para Petal.Length se mantiene entre ambos conjuntos de datos.
El MADM se mantiene para cada conjunto de datos.
El IQR es ligeramente más alto en el IQR.
...
```{r , include = FALSE}
# Regla 3 sigma
#Valores por encima de 3 desviaciones típicas se consideran outliers.

Regla_tres <- function(x) {
  x <- unlist(x)
  umbral <- 3 * sd(x, na.rm = TRUE)
  ifelse(abs(x - mean(x, na.rm = TRUE)) > umbral, TRUE, FALSE)
}

# Identificador Hampel
# Valores por encima de 3 MADM se consideran outliers.

Hampel <- function(x) {
  x <- unlist(x)
  umbral <- 3 * MADM(x)
  ifelse(x > umbral, TRUE, FALSE)
}
...
reglapercentil<-function(x){
  ...
  xu<-quantile(x, 0.05, na.rm = TRUE)
  xi<-quantile(x, 0.95, na.rm = TRUE)

  outliers<-if_else((x<xu)|(x>xi),T,F)
  return(outliers)
}
...
outliers_iris <- clean_iris %>%
  mutate(
    outlier_Regla3 = Regla_tres(Petal.Length),
    outlier_Hampel = Hampel(Petal.Length),
    outlier_Boxplot = reglaboxplot(Petal.Length),
    outlier_Percentil = reglapercentil(Petal.Length)
  )
```
...
iris_out <- clean_iris %>%
  ...
  # 2️⃣ Reorganizar con pivot_longer (de ancho a largo)
  pivot_longer(
    cols = starts_with("outlier_"),
    names_to = "Metodo",
    values_to = "Es_outlier"
  )

ggplot(iris_out, aes(x = Metodo, y = Petal.Length, color = Es_outlier)) +
  geom_point(position = position_jitter(width = 0.2, height = 0)) +
  scale_color_manual(values = c("FALSE" = "gray50", "TRUE" = "red")) +
  labs(
    title = "Detección de outliers en Petal.Length",
    subtitle = "Comparación entre métodos (Regla 3σ, Hampel, Boxplot, Percentil)",
    x = "Método de detección",
    y = "Petal.Length",
    color = "Outlier"
  ) +
